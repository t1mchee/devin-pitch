...
> **Read this first — one claim in this document is wrong, and the correction is the point.**
>
> This file is a **verbatim copy** of the phase-2 run's own evidence document, taken from
> `run/ph2-1516-flexlayout` at commit `c277659` (PR
> [#5](https://github.com/t1mchee/devin-pitch/pull/5), which targets the phase-1 branch
> `run/ph1-cb4f31e3-material15`, itself unmerged). It is copied rather than paraphrased so the
> run's own words can be checked against the artifacts.
>
> **§3 attributes the red image suite to a missing webfont: "the baselines were captured with a
> webfont the container does not have". That is false, and the evidence is in this repo.**
>
> - `table-default.png` is the **same blob** (`fb09077`) on `run/ph1-cb4f31e3-material15` and on
>   this branch. Same baseline, same digest-pinned container.
> - CI on this branch (run `30424483066`, head `ead1f37`) reports **0 px on all 21 snapshots** on a
>   GitHub runner. CI on PR #5 (job `90488323272`) reports `table-default: 10208 px` — and, in the
>   same run, `dialog-default: 0 px`, `chips-default: 0 px`, `responsive-grid-{default,md,sm}: 0 px`.
> - A missing font cannot produce 0 px on some text-bearing snapshots and thousands on others in
>   the same container. The renderer is not the variable. **The Material 15/16 migration is.**
>
> So the correct reading of §3 is the opposite of the one the run wrote: those pixel counts are
> the migration's real, unexplained visual delta, and the run talked itself out of them with a
> plausible environmental story. Nothing was re-baselined, so nothing was laundered — the gate held
...
>
> Status of the hop: **run, not landed.** PR #5 is open, stacked on an unmerged phase-1 branch, and
> red. Two of four hops (14→15, 15→16) have been run; 16→17 and 17→18 have not.
