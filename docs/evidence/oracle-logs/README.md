...
current **59-test** suite (23 image + 36 computed-style); the earlier logs below show the 46-test
light-only suite they were captured against, which is why their probe counts read 23.

| log | what it establishes | result |
|---|---|---|
| `noise-repeat-{1,2,3}.log` | renderer noise floor: three consecutive runs, unmodified tree | **0 px on all 21 snapshots**, 23 image + 36 computed-style tests green, three times |
