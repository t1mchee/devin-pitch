...
| Step 2 must run `npm run visual` (pinned digest, `--skip-nx-cache`), never a bare `nx e2e`. | A host run disagrees with the baselines by 455–5,963 px (`docs/evidence/ORACLE-noise-floor.md`) and an Nx cache hit is not a test run. Both mistakes were made during this build. |
